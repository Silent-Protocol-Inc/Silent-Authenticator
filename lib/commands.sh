...
Usage:
  ...
  sat.sh web-domain <domain> [--cloudflare dns-only|proxied] [--port PORT]
...
Security:
  ...
  `web-domain` publishes SAT only behind nginx HTTPS. It uses Certbot DNS-01
  with a restricted Cloudflare API token, then keeps SAT bound to localhost.
EOF
}
...
web_domain_owner_id() {
	local hash
	hash="$(printf '%s' "$SAT_HOME" | openssl dgst -sha256 2>/dev/null | awk '{print $NF}')"
	printf '%s' "${hash:0:16}"
}

web_domain_owner_id_from_vhost() {
	printf '%s\n' "$1" | sed -n 's/.*# SAT owner: \([0-9a-f]\{1,\}\).*/\1/p'
}

web_domain_vhost_owner_ok() {
	local existing_body="$1" requested_owner="$2" existing_owner
	existing_owner="$(web_domain_owner_id_from_vhost "$existing_body")"
	[[ -z "$existing_owner" || "$existing_owner" == "$requested_owner" ]]
}

web_domain_assert_owner() {
	local domain="$1" available existing_body
	available="/etc/nginx/sites-available/$domain"
	if ! sudo test -e "$available"; then return 0; fi
	if ! sudo grep -Fq '# Generated by Silent Authenticator Tool' "$available"; then
		fail 5 'domain_vhost_conflict' "nginx sudah memiliki vhost non-SAT untuk $domain; konfigurasi tidak ditimpa."
	fi
	existing_body="$(sudo cat "$available")"
	web_domain_vhost_owner_ok "$existing_body" "$(web_domain_owner_id)" || \
		fail 5 'domain_owner_conflict' "Hostname $domain sudah diikat oleh instance SAT lain; hentikan instance tersebut sebelum mengikat ulang hostname ini. Binding lama tidak ditimpa."
}

web_domain_assert_upstream_isolated() {
	local domain="$1" port="$2" enabled='/etc/nginx/sites-enabled' candidate conflict=''
	[[ -d "$enabled" ]] || return 0
	while IFS= read -r -d '' candidate; do
		[[ "$candidate" == "$enabled/$domain" ]] && continue
		if sudo grep -Fq "proxy_pass http://127.0.0.1:$port;" "$candidate"; then
			conflict="$candidate"
			break
		fi
	done < <(sudo find -L "$enabled" -maxdepth 1 -type f -print0)
	[[ -z "$conflict" ]] || fail 5 'origin_proxy_conflict' "Port origin 127.0.0.1:$port sudah diproksikan oleh vhost lain: $conflict. Lepaskan atau gunakan port origin lain agar hostname SAT tetap terisolasi."
}

web_domain_require_tools() {
	nginx_bin >/dev/null 2>&1 || fail 7 'nginx_missing' 'HTTPS domain membutuhkan nginx.'
	require_cmd certbot
	require_cmd sudo
...
web_domain_render_vhost() {
	local domain="$1" port="$2" proxied="$3" nginx_version
	nginx_version="$("$(nginx_bin)" -v 2>&1 | sed -n 's|.*nginx/\([0-9][0-9.]*\).*|\1|p')"
	printf '# Generated by Silent Authenticator Tool %s.\n' "$APP_VERSION"
	printf '# SAT owner: %s\n' "$(web_domain_owner_id)"
	...
	[[ "$proxied" == 'yes' ]] && printf '    include /etc/nginx/snippets/sat-cloudflare-realip.conf;\n'
...
web_domain_write_cloudflare_realip() {
	local temporary snippet='/etc/nginx/snippets/sat-cloudflare-realip.conf'
	temporary="$(mktemp "$SAT_PROCESS_TEMP_DIR/cloudflare-realip.XXXXXX")"
	{
		printf '# Generated by SAT. Trust Cloudflare only for its published address ranges.\n'
		curl -fsS --max-time 10 https://www.cloudflare.com/ips-v4 2>/dev/null | sed -e '/^$/d' -e 's/^/set_real_ip_from /' -e 's/$/;/' || true
		curl -fsS --max-time 10 https://www.cloudflare.com/ips-v6 2>/dev/null | sed -e '/^$/d' -e 's/^/set_real_ip_from /' -e 's/$/;/' || true
		printf 'real_ip_header CF-Connecting-IP;\n'
	} >"$temporary"
	rg -q '^set_real_ip_from ' "$temporary" || fail 8 'cloudflare_ip_ranges_unavailable' 'Rentang IP Cloudflare tidak dapat diambil; nginx tidak diubah.'
	sudo mkdir -p /etc/nginx/snippets
	sudo install -m 0644 "$temporary" "$snippet"
}

web_domain_install_vhost() {
	local domain="$1" body="$2" nginx_executable available enabled backup='' had_enabled='no'
	available="/etc/nginx/sites-available/$domain"; enabled="/etc/nginx/sites-enabled/$domain"
	nginx_executable="$(nginx_bin)" || fail 7 'nginx_missing' 'HTTPS domain membutuhkan nginx.'
	if sudo test -e "$available"; then
		web_domain_assert_owner "$domain"
		backup="$(mktemp "$SAT_PROCESS_TEMP_DIR/sat-vhost.XXXXXX")"
		sudo cp "$available" "$backup"
	fi
	[[ -L "$enabled" ]] && had_enabled='yes'
	sudo mkdir -p /etc/nginx/sites-available /etc/nginx/sites-enabled
	printf '%s\n' "$body" | sudo tee "$available" >/dev/null
	sudo chmod 0644 "$available"
	sudo ln -sfn "$available" "$enabled"
	if ! sudo "$nginx_executable" -t; then
		printf 'Konfigurasi nginx SAT tidak valid; melakukan rollback.\n' >&2
		sudo rm -f "$enabled"
		if [[ -n "$backup" ]]; then
			sudo cp "$backup" "$available"
			[[ "$had_enabled" == 'yes' ]] && sudo ln -s "$available" "$enabled"
		else
			sudo rm -f "$available"
		fi
		fail 8 'nginx_config_invalid' 'Konfigurasi nginx baru tidak valid; konfigurasi sebelumnya dipulihkan.'
	fi
	if systemctl is-active --quiet nginx 2>/dev/null; then sudo systemctl reload nginx; else sudo "$nginx_executable" -s reload; fi
}
...
web_domain_setup_tls() {
	...
	case "$cloudflare" in
		proxied) proxied='yes' ;;
		dns-only) ;;
		*) fail 6 'cloudflare_dns_required' 'Mode HTTPS domain SAT membutuhkan DNS Cloudflare untuk validasi TXT.' ;;
	esac
	web_domain_require_tools
	web_local_port_available "$port" || fail 8 'web_port_in_use' "Port localhost $port sedang dipakai; pilih port origin lain sebelum menerbitkan sertifikat."
	web_domain_assert_owner "$domain"
...
parse_web_options() {
	...
	while (($#)); do
		case "$1" in
			...
			--port) [[ $# -ge 2 ]] || fail 2 'usage' '--port membutuhkan nilai.'; WEB_PORT="$2"; shift 2 ;;
	...
	if [[ -n "$WEB_DOMAIN" ]]; then
		...
		case "$WEB_CLOUDFLARE" in dns-only|proxied) ;; *) fail 6 'cloudflare_dns_required' 'HTTPS domain SAT membutuhkan Cloudflare DNS dengan mode dns-only atau proxied.' ;; esac
		WEB_HOST='127.0.0.1'; WEB_DEPLOYMENT='domain'
	fi
	...
	[[ -z "$WEB_DOMAIN" ]] || web_local_port_available "$WEB_PORT" || fail 8 'web_port_in_use' "Port localhost $WEB_PORT sedang dipakai; pilih port origin lain sebelum memasukkan token atau master password."
...
run_web_server() {
	...
	if [[ -n "$WEB_DOMAIN" ]]; then web_domain_setup_tls "$WEB_DOMAIN" "$WEB_PORT" "$WEB_CLOUDFLARE"; fi
	...
	[[ -z "$WEB_DOMAIN" ]] || server+=(--allowed-host "$WEB_DOMAIN")
	...
	if [[ -n "$WEB_DOMAIN" ]]; then web_url="https://$display_host"; else web_url="$(web_display_url "$display_host" "$WEB_PORT")"; fi
...
cmd_web_status() {
	local pid host='' port='' display_host='' deployment='local' cloudflare='off' url='' allowed_host=''
	...
	if [[ -f "$SAT_WEB_STATE_FILE" ]]; then
		...
		if validate_host "$host" && validate_port "$port"; then
			...
			if [[ "$deployment" == 'domain' ]]; then url="https://$display_host"; allowed_host="$display_host"; else url="$(web_display_url "$display_host" "$port")"; fi
		fi
	fi
	if [[ -n "$url" ]] && web_health_check "$host" "$port" "$allowed_host"; then
